...
.config(['$analyticsProvider', '$windowProvider', function ($analyticsProvider, $windowProvider) {
  ...
  /**
   * Add a Tracker ID for all the next dispatch to GA
   * @name pageTrack
   *
   * @param {string} ID of the tracker
   *
   * @link https://developers.google.com/analytics/devguides/collection/analyticsjs/sending-hits
   */
  $analyticsProvider.registerTrackerId = function (pTrackerId) {
      if (typeof pTrackerId === "string" && pTrackerId.length) {
          trackerId = pTrackerId;
      }
  };

  /**
   * Add the tracker ID to the GA command if any
   * @name pageTrack
   *
   * @param {string} command of the GA
   *
   * @returns {string} command string with or without the tracker depending if the tracker ID is set
   */
  function getUACommand(command) {
    if (typeof trackerId === "string" && trackerId.length) {
      return trackerId + "." + command;
    }
    ...
    function dispatchToClassic_(command, fieldsObj, shouldCopyHit) {
      ...
      if (command === 'send') {
        ...
        switch(fieldsObj.hitType) {
          case 'pageview':
            classicCommand = [
              getUACommand('_trackPageview'),
              fieldsObj.page
            ];
            break;
          case 'event':
            classicCommand = [
              getUACommand('_trackEvent'),
